...
def setup_rag_chain_from_file(file_path):
    ...
    try:
        ...
        rag_prompt_template = """
        SYSTEM PERSONAL: You are SwasthAI, a prescription analysis expert.
        ...
        **Guidance:**
        ...
        import traceback
        traceback.print_exc()
        print("ERROR OCCURRED")   # 👈 add this also
        return False
    ...
    try:
        ...
        if chat_stage == 'PREDICTION_STARTED':
            ...
            **When to See a Doctor:** (List critical symptoms that require immediate medical attention.)
            """
            ai_response = llm.invoke(prompt[:1000])
            session['last_prediction_result'] = ai_response[:2000]
...
def analyze_prescription():
    ...
    try:
        ...
        session['last_analysis_result'] = analysis_result[:2000]
